...
// derived CvarSetting that supports min and max for float values and a precision parameter to indicate the step size
function CvarSettingSlider(name, cvar, min, max, precision) {
	// these options are set before calling CvarSettings so that readValue isn't called before precision has been set
	var step = 0.1;
	
	if(typeof precision !== 'undefined')
	{
		// calculate the step value based on the precision value specified
		step = Math.pow(10, -precision);
	}
	
	this.step = step;
	this.precision = typeof precision !== 'undefined' ? precision : 1;
...
CvarSettingSlider.prototype.readValue = function() {
	...
	engine.call("getCvarFloat", this.cvar).then(function(value) {
		outer.value(value.toFixed(outer.precision));
	});	
...
function MainMenuViewModel() {
	...
	this.options = [
		...
		new OptionGroup( "#GameUI_GameSettings", true,
			[
                new CvarSettingCheckbox( "#GameUI_RelativeTeamColors", "cl_team_colors_relative" ),
                new CvarSettingCheckbox( "#GameUI_AlwaysDisplayCompass", "cl_hud_compass_show_always" ),
				new CvarSettingCheckbox( "#GameUI_AutoBipod", "cl_bipod_auto" ),
				new CvarSettingCheckbox( "#GameUI_AutoSwitchAfterGrenade", "cl_grenade_auto_switch" ),
		...
		new OptionGroup( "#GameUI_Video", false,
			[
				new CvarSettingSlider( "#GameUI_FieldOfView", "fov_desired", 75, 110, 0 ),
				...
				new OptionRowSubsection( "#GameUI_VideoOptions_Experimental" ),
				
				new OptionRowSubsection( "#GameUI_VideoOptions_AmbientOcclusion" ),
				new CvarSettingCheckbox( "#GameUI_Enabled", "mat_hbao" ),
				new CvarSettingSlider( "#GameUI_VideoOptions_AmbientOcclusion_Intensity", "mat_hbao_occlusionintensity", 0.0, 5.0 ),
				
				new OptionRowSubsection( "#GameUI_VideoOptions_FilmGrain" ),
				new CvarSettingCheckbox( "#GameUI_Enabled", "mat_rgbfilmgrain" ),
				new CvarSettingCheckbox( "#GameUI_VideoOptions_FilmGrain_SpectatorOnly", "mat_rgbfilmgrain_spectatoronly" ),
				new CvarSettingSlider( "#GameUI_VideoOptions_FilmGrain_Intensity", "mat_rgbfilmgrain_intensity", 0.0, 3.0 ),
				new CvarSettingSlider( "#GameUI_VideoOptions_FilmGrain_Falloff", "mat_rgbfilmgrain_falloff", 0.0, 16.0 ),
				new CvarSettingSlider( "#GameUI_VideoOptions_FilmGrain_Size", "mat_rgbfilmgrain_size", 128.0, 512.0, 0 ),
				
				new OptionRowSubsection( "#GameUI_VideoOptions_VolumetricScattering" ),
				new CvarSettingCheckbox( "#GameUI_Enabled", "r_volumetricsunshafts" ),
				new CvarSettingCheckbox( "#GameUI_VideoOptions_VolumetricScattering_HighQuality", "r_volumetricsunshafts_fullresolutionpass" ),
				new CvarSettingSlider( "#GameUI_VideoOptions_VolumetricScattering_Intensity", "r_volumetricsunshafts_intensity", 0.0, 0.01, 4 ),
				new CvarSettingSlider( "#GameUI_VideoOptions_VolumetricScattering_Scattering", "r_volumetricsunshafts_scattering", 0.0, 1.0 ),
			]
		),
